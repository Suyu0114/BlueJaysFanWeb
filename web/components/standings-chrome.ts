 ...
 * depending on how Tailwind happened to emit the rules.
 */
export function rowBg(row: StandingsRow, index: number): string {
  if (row.team_id === TORONTO_TEAM_ID) return "bg-brick/20 font-medium";
  return index % 2 === 0 ? "bg-papaya/70" : "bg-papaya/35";
}
